...
function App() {
  ...
  useEffect(()=>{
    ...
    async function fetchData(){
      let response = await fetch(`https://api.mfapi.in/mf/search?q=${seachedMF}`);
  ...
  useEffect(()=>{
    ...
    async function fetchHistoricalNav(){
      let response = await fetch(`https://api.mfapi.in/mf/${selectedMFSchemeCode}`);
  ...
  useEffect(()=>{
    if(historicalNav.length>0 && fromSIPDate && toSIPDate){
      ...
      while (ptr.nextPtr !== null) {
        ...
        console.log('currentSIPDate',moment(currentSIPDate).format("DD-MM-YYYY"))
        console.log('check',ptr.value.date === moment(currentSIPDate).format("DD-MM-YYYY"));
        console.log("moment(ptr.value.date",moment(ptr.value.date,"DD-MM-YYYY").get('date'));
        console.log('fromSIPDate',fromSIPDate);
        console.log('toSIPDate',toSIPDate);
        console.log('moment(toSIPDate,"DD-MM-YYYY").diff(moment(ptr.value.date,"DD-MM-YYYY"))',moment(toSIPDate,"DD-MM-YYYY").diff(moment(ptr.value.date,"DD-MM-YYYY")))
        if(moment(toSIPDate,"YYYY-MM-DD").diff(moment(ptr.value.date,"DD-MM-YYYY"))>0 && ((ptr.value.date === moment(currentSIPDate).format("DD-MM-YYYY")) || moment(ptr.value.date,"DD-MM-YYYY").diff(moment(currentSIPDate))>0)){
        ...
        console.log('moment(ptr.value.date,"DD-MM-YYYY").diff(moment(currentSIPDate))',moment(ptr.value.date,"DD-MM-YYYY").diff(moment(currentSIPDate)))
        ptr = ptr.nextPtr;
      }
  ...
  const mfUnits = filterHistoricalNav.reduce((acc,item)=>{
    ...
    console.log("moment diff",currentDate.diff(SELL_DATE),currentDate.diff(moment(LAST_YEAR_DATE,"DD-MM-YYYY")));
 
                     if(currentDate.diff(SELL_DATE)<=0){
                      if(currentDate.diff(moment(LAST_YEAR_DATE,"DD-MM-YYYY"))>0){
                        acc = {...acc,stcgUnits:acc.stcgUnits + Number(sipAmount/item.nav),stcgInvestmentAmount: Number(sipAmount) + acc.stcgInvestmentAmount}
                       }
                       else{
                        acc = {...acc,ltcgUnits:acc.ltcgUnits + Number(sipAmount/item.nav),ltcgInvestmentAmount: Number(sipAmount) + acc.ltcgInvestmentAmount}
                       }
                     }
                      return {...acc,totalUnits:acc.totalUnits + Number(sipAmount/item.nav)};
                      },{ltcgUnits:0,stcgUnits:0,ltcgInvestmentAmount:0,stcgInvestmentAmount:0,totalUnits:0});
  ...
  if(filterHistoricalNav.length>0){
    ...
    while (ptr.nextPtr !== null && sellDate) {
      if(((ptr.value.date === moment(SELL_DATE).format("DD-MM-YYYY")) || moment(ptr.value.date,"DD-MM-YYYY").diff(moment(SELL_DATE))>0)){
    ...
    totalProfit = sellDate ? (mfUnits.totalUnits*sellDateNav)-totalInvestment: 0;
    ...
    <div className="App">
      <main>
        <div>
           ...
           {SIPDATES.map((sip,index)=><option key={sip+index} value={index+1}>
                {index+1}
              </option>)}
            </select> */}
            <label>
        ...
        <div>
          <div>Total Investments: <span>{totalInvestment}</span></div>
          <div>Total units: <span>{mfUnits.totalUnits}</span></div>
          <div>Total LTCG units: <span>{mfUnits.ltcgUnits}</span></div>
          <div>Total STCG units: <span>{mfUnits.stcgUnits}</span></div>
          <div>Total profit: <span>{totalProfit}</span></div>
          <div>Total LTCG: <span>{totalLTCG}</span></div>
          <div>Total STCG: <span>{totalSTCG}</span></div>
